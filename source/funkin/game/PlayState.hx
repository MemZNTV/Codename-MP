	...
	**/
	...
	@:dox(hide) override public function create()
	{
		...
		for(i=>strumLine in SONG.strumLines) {
			...
			// multiplayer: the strumline nobody here controls (and isn't the additional/gf one) is played by the other person over the network
			strLine.remote = MultiplayerMatch.active && strumLine.type != 2 && strLine.cpu;
	...
	@:dox(hide) public override function createPost() {
		// multiplayer: no cutscene, and the countdown only starts once the server says both players are ready
		if (MultiplayerMatch.active) MultiplayerMatch.onPlayStateReady(this);
		else startCutscene("", cutscene, null, true);
		super.createPost();
	...
	override public function update(elapsed:Float)
	{
		...
		if (MultiplayerMatch.active) MultiplayerMatch.update(this, elapsed);
		...
		if (startedCountdown && controls.RESET && !MultiplayerMatch.active)
			gameOver();
	...
	**/
	...
	public function nextSong() {
		if (MultiplayerMatch.active) {
			MultiplayerMatch.onSongFinished(this);
			return;
		}
	...
	public function noteMiss(strumLine:StrumLine, note:Note, ?direction:Int, ?player:Int):Void
	{
		...
		// a miss on a remote (multiplayer) strumline only affects health/animations, never the local player's score, combo or the shared vocals
		var remote = strumLine != null && strumLine.remote;

		var event:NoteMissEvent = gameAndCharsEvent(remote ? "onDadMiss" : "onPlayerMiss", EventManager.get(NoteMissEvent).recycle(note, -10, 1, muteVocalsOnMiss, hasNote ? ((note.isSustainNote && Flags.SUSTAINS_AS_ONE_NOTE) ? -0.1425 : -0.0475) : -0.04, Paths.sound(FlxG.random.getObject(Flags.DEFAULT_MISS_SOUNDS)), FlxG.random.float(0.1, 0.2), !hasNote, combo > 5, "sad", true, true, "miss", strumLines.members[playerID].characters, playerID, hasNote ? note.noteType : null, directionID, 0));
		strumLine.onMiss.dispatch(event);
		if (event.cancelled) {
			gameAndCharsEvent(remote ? "onPostDadMiss" : "onPostPlayerMiss", event);
			return;
		}

		if (strumLine != null) strumLine.addHealth(event.healthGain);
		if (!remote && gf != null && event.gfSad && gf.hasAnimation(event.gfSadAnim))
			gf.playAnim(event.gfSadAnim, event.forceGfAnim, MISS);

		if (event.resetCombo && !remote) combo = 0;
		...
		if (event.playMissSound && !remote) FlxG.sound.play(event.missSound, event.missVolume);

		if (event.muteVocals) {
			if (!remote) vocals.volume = 0;
			strumLine.vocals.volume = 0;
		}

		if (event.accuracy != null && !remote) {
	 ...
	 * @param overrideDiff (Optional) Use this timing difference (in ms) instead of the current song position. Used by multiplayer to replay another player's hit exactly as they judged it.
	 */
	public function goodNoteHit(strumLine:StrumLine, note:Note, ?overrideDiff:Float):Void
	{
		if(note == null || note.wasGoodHit) return;

		note.wasGoodHit = true;

		var noteDiff = overrideDiff != null ? overrideDiff : Math.abs(Conductor.songPosition - note.strumTime), rating:Rating;
		...
		if (strumLine != null && !strumLine.cpu)
			...
			// remote (multiplayer) hits move the health like a real player's would, but must never count as a local miss
			var remote = strumLine != null && strumLine.remote;
			event = EventManager.get(NoteHitEvent).recycle(remote ? false : rating.breaksCombo, false, false, null, null, null, note, strumLine.characters, false, note.noteType, note.animSuffix.getDefault(note.strumID < strumLine.members.length ? strumLine.members[note.strumID].animSuffix : strumLine.animSuffix), null, null, note.strumID, 0, null, remote ? rating.health : 0, rating.name, false, null, null, null, null, true, iconP2, false);
		}
		...
		if (event.unmuteVocals) {
			if (!strumLine.remote) vocals.volume = 1; // the shared vocals only follow the local player
