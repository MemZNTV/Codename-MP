...
class StrumLine extends FlxTypedGroup<Strum> {
	...
	/**
	 * Signal that triggers whenever a key of this strumline gets pressed or released by the player.
	 * Arguments are the strum id and whenever it was pressed (`true`) or released (`false`).
	 * Used by multiplayer to mirror the keys on the other player's screen.
	 */
	public var onKeyChange:FlxTypedSignal<Int->Bool->Void> = new FlxTypedSignal<Int->Bool->Void>();
	/**
	 * Whenever this strumline is played by another person over the network (multiplayer).
	 * A remote strumline is also `cpu`, but its notes are NOT auto-hit: they only get hit or missed when
	 * the other player's client says so (see `funkin.multiplayer.MultiplayerMatch`).
	 */
	public var remote:Bool = false;
	/**
	 * How long (in ms) a note of a remote strumline stays around after its hit window passed,
	 * waiting for the other player's hit/miss message to arrive. Only used to clean up if a message got lost.
	 */
	public static inline final REMOTE_NOTE_GRACE:Float = 1500;
	/**
	...
	**/
	public function updateNote(daNote:Note) {
		...
		if (cpu && !remote && __updateNote_event.__autoCPUHit && !daNote.avoid && !daNote.wasGoodHit && daNote.strumTime < __updateNote_songPos)
		...
		if (daNote.tooLate) {
			if (remote) {
				// what happens to a remote note is decided by the other player, so keep it (still scrolling)
				// until their hit/miss message arrives. Only clean up if it never does.
				if (__updateNote_songPos - daNote.strumTime > REMOTE_NOTE_GRACE) {
					deleteNote(daNote);
					return;
				}
			} else {
	...
	**/
	public function updateInput(id:Int = 0) {
		...
		// before any note gets hit, so listeners see the key press first
		for (i in 0...membersLength) {
			if (__justPressed[i] == true) onKeyChange.dispatch(i, true);
			else if (__justReleased[i] == true) onKeyChange.dispatch(i, false);
		}
	...
	**/
	...
	public function createStrum(i:Int, ?animPrefix:String, ?spritePath:String, ?playIntroAnimation:Bool) {
		...
		babyArrow.cpu = cpu && !remote; // remote strums animate like a player's (held keys stay glowing)
