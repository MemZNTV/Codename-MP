...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		// Versus multiplayer entry. Added here (not only in menuItems.txt) so it shows up in every mod.
		if (!optionShit.contains("multiplayer"))
			optionShit.insert(optionShit.contains("freeplay") ? optionShit.indexOf("freeplay") + 1 : optionShit.length, "multiplayer");
	...
	function selectItem() {
		...
		FlxFlicker.flicker(menuItems.members[curSelected], 1, Options.flashingMenu ? 0.06 : 0.15, false, false, function(flick:FlxFlicker)
		{
			...
			switch (event.name)
			{
				...
				case 'multiplayer': FlxG.switchState(new funkin.multiplayer.MultiplayerMenuState());
